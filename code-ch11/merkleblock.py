...
class MerkleTree:
    ...
    def populate_tree(self, flag_bits, hashes):
        ...
        if len(hashes) != 0:  # <12>
            raise RuntimeError(
                'hashes not all consumed {}'.format(len(hashes)))
...
class MerkleBlock:
    ...
    def parse(cls, s):
        ...
        number_of_transaction_hashes = read_varint(s)
        # each transaction is 32 bytes, Little-Endian
        each_transaction = [s.read(32)[::-1]
                            for _ in range(number_of_transaction_hashes)]
        # length of flags field - varint
        length_of_flags = read_varint(s)
        # read the flags field
        flags = s.read(length_of_flags)
        # initialize class
        return cls(version, prev_block, merkle_root, timestamp, bits, nonce, total_transactions, each_transaction, flags)
    ...
    def is_valid(self):
        ...
        tree.populate_tree(bits, hashes)
